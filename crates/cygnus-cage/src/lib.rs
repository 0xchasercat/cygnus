...
impl Cage {
    /// Boot a target in a new cage.
    pub fn boot(_spec: CageSpec) -> Result<Self, CageError> {
        Err(CageError::NotImplemented)
    }
...
pub enum CageError {
    ...
    InvalidSpec(String),
    #[error("cage boot is not implemented")]
    NotImplemented,
}

fn validate_name(name: &str) -> Result<(), CageError> {
